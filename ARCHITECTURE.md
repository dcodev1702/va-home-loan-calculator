...
Every image build produces a **CycloneDX** SBOM committed to the repo under
`sbom/sentinel-va-<version>.cdx.json`, generated by
[Syft](https://github.com/anchore/syft) via `scripts/generate-sbom.sh <version>`.
CycloneDX (ECMA-424) is chosen over SPDX for its security/DevSecOps focus and native
fit with the Anchore tooling; both are equally accepted by CISA/NTIA, the EU CRA, and
FDA guidance — the guidance is to pick one, and Syft can emit SPDX on demand if ever
requested. The SBOM is a **point-in-time snapshot of a specific image digest**, so it
is regenerated on EVERY rebuild as part of the release SOP (§10) — a stale SBOM is
worse than none. Vulnerability scanning is done separately with Docker Scout (and
grype); the SBOM is the inventory, not the scan.

### 9.6 Local run paths
- **Source:** `run.sh` (Node 20+, no Docker).
- **Prebuilt:** `docker run` of `digitalkali/sentinel-va:latest` (see `RUNNING.md`).

### 9.7 Public sharing — Cloudflare quick tunnel
...
6. **Generate + commit the SBOM** for the new image: `./scripts/generate-sbom.sh
   X.Y.Z` → `sbom/sentinel-va-X.Y.Z.cdx.json` (CycloneDX; §9.5). Regenerate on every
   rebuild so it never drifts from the shipped digest.
7. **Redeploy** the live container on the new image with the identical hardened run
   config, republishing the **same host port** — **never touch cloudflared** (the
   quick-tunnel URL must stay stable between rotations).
8. **Remove the previous on-disk image** (`docker rmi <repo>:PREV`).

Steps 5–8 are skipped for changes that don't affect the built artifact (CI-config- or
